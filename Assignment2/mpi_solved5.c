...
/*
 Solution: MPI_Send sends an infinite number of data to process 1, and there is 
 no proper mechanism to ensure the messages get received before the next send. 
 This will exhaust the buffer, so one wants to make sure the data sent are 
 appropriately received. We use MPI_Ssend instead.
 */
